...
class SqlQueriesPerViewTests(AssertNumQueriesMixin, TestCase):
    ...
    def test_edit_plant_details_endpoint(self):
        '''/edit_plant_details should make 5 database queries.'''
        plant = Plant.objects.create(uuid=uuid4(), user=get_default_user())
        with self.assertNumQueries(5):
    ...
    def test_bulk_delete_plants_and_groups_endpoint_1_plant(self):
        '''/bulk_delete_plants_and_groups should make 15 database queries when
        deleting a single Plant instance.
        '''
        plant = Plant.objects.create(uuid=uuid4(), user=get_default_user(), name='Plant 1')
    ...
    def test_bulk_delete_plants_and_groups_endpoint_3_plants(self):
        ...
        plant2 = Plant.objects.create(uuid=uuid4(), user=get_default_user(), name='Plant 2')
        plant3 = Plant.objects.create(uuid=uuid4(), user=get_default_user(), name='Plant 3')
        with self.assertNumQueries(15):
            response = self.client.post('/bulk_delete_plants_and_groups', {
                'uuids': [
                    str(plant1.uuid),
    ...
    def test_bulk_delete_plants_and_groups_endpoint_plant_in_group(self):
        ...
        '''
        user = get_default_user()
        group1 = Group.objects.create(uuid=uuid4(), user=user, name='Group 1')
        group2 = Group.objects.create(uuid=uuid4(), user=user, name='Group 2')
        group3 = Group.objects.create(uuid=uuid4(), user=user, name='Group 3')
    ...
    def test_bulk_archive_plants_and_groups_endpoint_plants(self):
        '''/bulk_archive_plants_and_groups should make 7 database queries when
        archiving any number of Plant instances.
        '''
        ...
        plant4 = Plant.objects.create(uuid=uuid4(), user=get_default_user())

        # Confirm makes 7 queries when 1 Plant is archived
        with self.assertNumQueries(7):
        ...
        # Confirm makes 7 queries when 3 Plants are archived
        with self.assertNumQueries(7):
            response = self.client.post('/bulk_archive_plants_and_groups', {
                'uuids': [
                    str(plant2.uuid),
    ...
    def test_bulk_archive_plants_and_groups_endpoint_groups(self):
        '''/bulk_archive_plants_and_groups should make 5 database queries when
        archiving any number of Group instances.
        '''
        ...
        group4 = Group.objects.create(uuid=uuid4(), user=get_default_user())

        # Confirm makes 5 queries when 1 Group is archived
        with self.assertNumQueries(5):
            response = self.client.post('/bulk_archive_plants_and_groups', {
                'uuids': [
                    str(group1.uuid)
        ...
        # Confirm makes 5 queries when 3 Groups are archived
        with self.assertNumQueries(5):
            response = self.client.post('/bulk_archive_plants_and_groups', {
                'uuids': [
                    str(group2.uuid),
    ...
    def test_bulk_archive_plants_and_groups_endpoint_plants_and_groups(self):
        '''/bulk_archive_plants_and_groups should make 8 database queries when
        ...
        # Confirm makes 8 queries when 1 Plant and 1 Group are archived
        with self.assertNumQueries(8):
            response = self.client.post('/bulk_archive_plants_and_groups', {
                'uuids': [
                    str(plants[0].uuid),
        ...
        # Confirm makes 8 queries when 3 Plants and 3 Group are archived
        with self.assertNumQueries(8):
            response = self.client.post('/bulk_archive_plants_and_groups', {
                'uuids': [
                    str(plants[1].uuid),
    ...
    def test_bulk_add_plants_to_group_endpoint(self):
        ...
        # Confirm makes 7 queries when 2 Plants added to Group
        with self.assertNumQueries(7):
            response = self.client.post('/bulk_add_plants_to_group', {
                'group_id': group.uuid,
                'plants': [
    ...
    def test_bulk_remove_plants_from_group_endpoint(self):
        '''/bulk_remove_plants_from_group should make 7 database queries
        ...
        # Confirm makes 7 queries when 2 Plants removed from Group
        with self.assertNumQueries(7):
            response = self.client.post('/bulk_remove_plants_from_group', {
                'group_id': group.uuid,
                'plants': [
    ...
    def test_repot_plant_endpoint(self):
        ...
        # Confirm 6 queries if new_pot_size is given
        with self.assertNumQueries(6):
            response = self.client.post('/repot_plant', {
                'plant_id': plant.uuid,
                'timestamp': '2024-02-06T03:06:27.000Z',
                'new_pot_size': 6
            })
            self.assertEqual(response.status_code, 200)
